...
{
    private readonly List<CSharpClassBase> _classes = [];
    ...
    internal CSharpClassBase[] Classes => _classes.ToArray();
    ...
    internal CSharpClass AddClass(string name)
    {
        ...
        {
            return (CSharpClass)_classes.First(_ => _.Name == name);
        }
        var c = new CSharpClass(name, this);
        _classes.Add(c);
        return c;
    }
    internal CSharpRecord AddRecord(string name)
    {
        if (_classes.Any(_ => _.Name == name))
        {
            return (CSharpRecord)_classes.First(_ => _.Name == name);
        }
        var c = new CSharpRecord(name, this);
        _classes.Add(c);
        return c;
    }
}
